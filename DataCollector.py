...
def create_gui():
    ...
    random_beep_var = tk.BooleanVar()
    random_beep_var.set(False)
    random_beep_checkbox = tk.Checkbutton(gui, text="Atsitiktinis garsinis signalas", variable=random_beep_var)
    random_beep_checkbox.pack()
        ...
        try:
            ...
            with EEGManager() as mgr:
                for x in range(loop_count):
                    ...
                    if beep_var.get():
                        if random_beep_var.get():
                            beep_time = random.uniform(0.5, sleep_duration - 1.5)
                            time.sleep(beep_time)
                            play_beep()
                            time.sleep(sleep_duration - beep_time)
                        else:
                            play_beep()
